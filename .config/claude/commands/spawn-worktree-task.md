---
description: 指示を元にpromptファイルを作成し、worktreeを作成してタスクを開始
allowed-tools: Write, Bash(twig add:*), Bash(code:*), Bash(git submodule:*)
...
`twig add --carry --file`で新しいworktreeに転送、最後にVS Codeで開く。
...
- `with /command` 形式でコマンドを指定すると、prompt内でそのコマンドを呼び出す
  - 例: `/spawn-worktree-task feat/new with /prepare-pr`
...
!`find . -maxdepth 1 -name "*.code-workspace" 2>/dev/null | head -1`
...
worktree名を含めることで、複数のspawn-worktree-taskを並列実行しても
ファイル名が衝突しない。`.twig-claude-prompt-*.sh`にマッチするファイルが
worktree内に存在する場合、VS Codeで開いた際にClaude Codeが自動的に起動し、
このファイルを実行する仕組みになっている。
...
### 4. promptファイル作成・worktree作成・VS Codeで開く
...
ファイル名にworktree名を含めることで、複数のspawn-worktree-taskを並列実行しても
ファイル名が衝突しない。
...
ファイル書き込み完了後、作成されたworktreeをVS Codeで開く。

コンテキストの「workspaceファイル」で確認した`.code-workspace`ファイルがあれば
`code <worktree-path>/<workspace-filename>`、なければ`code <worktree-path>`で開く。
